...
from utils.load import update_YFCC_config, load_cv_img_resize, update_ScanNet_config, update_MP3D_config, update_KITTI_config, update_MegaDepth_config
from utils.geo import recover_corrs_offset_scales, Homo_2d_pts, cal_corr_F_and_mean_sd, calc_sampson_dist
from utils.img_process import img_crop_direct, img_crop_with_resize, img_crop_fix_aspect_ratio
from utils.vis import plot_matches_lists_ud
from utils.comm import test_dir_if_not_create

class PRGeoAreaMatcher(object):
    ...
    ## Funcs:
        ...
        2. Rejector
    """
    def __init__(self, ORConfigs={}, datasetName="ScanNet", draw_verbose=0) -> None:
        """ initial with imgs
        """

        self.reject_outarea = False

        self.datasetName = datasetName
        if datasetName == "ScanNet":
            ORConfigs = update_ScanNet_config(ORConfigs)
        elif datasetName == "MatterPort3D":
            ORConfigs = update_MP3D_config(ORConfigs)
        elif datasetName == "KITTI":
            ORConfigs = update_KITTI_config(ORConfigs)
        elif datasetName == "MegaDepth":
            ORConfigs = update_MegaDepth_config(ORConfigs)
        elif datasetName == "YFCC":
            ORConfigs = update_YFCC_config(ORConfigs)
        else:
            raise NotImplementedError

        self.area_from_size_W, self.area_from_size_H = ORConfigs["area_from_size_W"], ORConfigs["area_from_size_H"]
        self.std_size = ORConfigs["standard_area_size"]
        self.crop_size_W, self.crop_size_H = ORConfigs["crop_size_W"], ORConfigs["crop_size_H"]
        self.crop_from_size_W, self.crop_from_size_H = ORConfigs["crop_from_size_W"], ORConfigs["crop_from_size_H"]
        self.eval_from_size_W, self.eval_from_size_H = ORConfigs["eval_from_size_W"], ORConfigs["eval_from_size_H"]

        # imgs ara loaded for area crop
        # if self.datasetName == "MegaDepth":
        if self.datasetName == "MegaDepth" or self.datasetName == "YFCC":
            # resize and padding, update the crop/eval_from_size
            self.ori_img0 = cv2.imread(ORConfigs["color_path0"], cv2.IMREAD_COLOR)
            ...
            self.ori_img1 = cv2.imread(ORConfigs["color_path1"], cv2.IMREAD_COLOR)
            ...
            self.ori_img0, self.img_scale0 = load_cv_img_resize(ORConfigs["color_path0"], self.crop_from_size_W, self.crop_from_size_H, mode=1)
            self.ori_img1, self.img_scale1 = load_cv_img_resize(ORConfigs["color_path1"], self.crop_from_size_W, self.crop_from_size_H, mode=1)

        self.std_match_num = ORConfigs["match_num"]
        self.rejection_mode = ORConfigs["rejection_mode"]
        self.inlier_thd_mode = ORConfigs["inlier_thd_mode"]
        
        self.out_path = ORConfigs["out_path"] # including scene name and pair name
        # self.out_path = os.path.join(ORConfigs["out_path"], ORConfigs["scene_name"]+"_"+ORConfigs["pair0"]+"_"+ORConfigs["pair1"])
    
        self.ori_doubt_areas0 = []
        self.ori_doubt_areas1 = []

        self.load_PMatcher_flag = False
        self.load_coarse_PMatcher_flag = False
        self.predict_AM_flag = False

        self.draw_verbose = draw_verbose
                
        self.alpha_list = ORConfigs["alpha_list"] # phi list @Eason 23-04-06
        assert type(self.alpha_list) == list, f"Not a list for phis: {self.alpha_list}"

        self.filter_area_num = ORConfigs["filter_area_num"]
        self.adaptive_size_thd = ORConfigs["adaptive_size_thd"]
        # self.draw_adap = ORConfigs["draw_adap"]
    ...
    def load_matcher(self, matcher):
        """
        """
        logger.info(f"load inited mather")
        self.load_PMatcher_flag = True
        self.matcher = matcher
    
    def load_coarse_matcher(self, matcher):
        """
        """
        logger.info(f"load inited coarse mather")
        self.load_coarse_PMatcher_flag = True
        self.coarse_matcher = matcher
    ...
    def rejection_by_samp_dist_flow(self, matched_area0s, matched_area1s):
        ...
        if len(self.rejecting_all_corrs) > 1:
            for i, src_corrs_ in enumerate(self.rejecting_all_corrs):
                ...
                if len(src_corrs) < 0.1 * self.std_match_num:
        ...
        for alpha_temp in self.alpha_list:
            ...
            if size_ratio0 <= self.adaptive_size_thd or size_ratio1 <= self.adaptive_size_thd:
                ...
                if len(areas_corrs_in_a_list) <= self.std_match_num * 0.1:
                    logger.warning(f"area corrs in alpha {alpha_temp} is too less, {len(areas_corrs_in_a_list)} < {self.std_match_num * 0.2}")
        ...
        return alpha_corrs_dict, self.best_F, alpha_idxs_dict
    ...
    def stack_corrs(self, corr_list):
        ...
        for i, corrs in enumerate(corr_list):
            if len(corrs) >= 0.2 * self.std_match_num:
                valid_idxs.append(i)
